...
Now includes group posting functionality
"""
...
SPRING_URI = os.getenv('SPRING_URI', 'http://localhost:8085')


# Helper functions for group integration
def get_spring_user_id(flask_user):
    """Map Flask user to Spring person ID using UID"""
    try:
        if not flask_user or not hasattr(flask_user, 'uid'):
            return None
            
        uid = flask_user.uid
        
        # Get person from Spring using UID
        response = requests.get(
            f"{SPRING_URI}/api/person/uid/{uid}",
            timeout=5
        )
        
        if response.ok:
            person = response.json()
            if person and person.get('uid') == uid:
                return person.get('id')
        
    except Exception as e:
        print(f"Error mapping Flask user to Spring ID: {e}")
    
    return None


def check_group_membership(spring_user_id, group_id):
    """Check if user is a member of the group and has posting permission"""
    try:
        # Get group member info from Spring
        response = requests.get(
            f"{SPRING_URI}/api/group/{group_id}/members",
            timeout=5
        )
        ...
        if response.ok:
            ...
            for member in members:
                if member.get('personId') == spring_user_id or member.get('id') == spring_user_id:
                    # Check if user has posting permission
                    can_post = member.get('canPost', True)
                    is_admin = member.get('role') == 'admin'
                    return can_post or is_admin
...
def get_group_info(group_id):
    """Fetch group information from Spring backend"""
    try:
        response = requests.get(
            f"{SPRING_URI}/api/group/{group_id}",
            timeout=5
        )
...
class MicroBlogAPI:
    ...
    class _CRUD(Resource):
        ...
        def get(self):
            ...
            filter_type = request.args.get('filter')  # NEW: 'user', 'group', or None
          
            try:
                if search:
                    microblogs_data = MicroBlog.search_content(search, limit)
                elif topic_id:
                    microblogs_data = MicroBlog.get_by_topic(topic_id, limit)
                elif page_path:
                    topic = Topic.get_by_page_path(page_path)
                    if topic:
                        microblogs_data = MicroBlog.get_by_topic(topic.id, limit)
                    else:
                        microblogs_data = []
                elif user_id:
                    microblogs_data = MicroBlog.get_by_user(user_id, limit)
                else:
                    microblogs_data = MicroBlog.get_all(limit)
                
                # NEW: Enrich posts with group information and apply filters
                enriched_posts = []
                for post in microblogs_data:
                    ...
                    post_data = post_dict.get('data', {}) if isinstance(post_dict, dict) else {}
                    is_group_post = post_data.get('author_type') == 'group'
                    
                    # Enrich with group info if it's a group post
                    if is_group_post:
                        group_id = post_data.get('author_id')
                        if group_id:
                    ...
                    if filter_type == 'group':
                        # Only include group posts
                        if is_group_post:
                            enriched_posts.append(post_dict)
                    elif filter_type == 'user':
                        # Only include user posts (not group posts)
    ...
    class _GroupPost(Resource):
        """Handle group posts"""
        
        @token_required()
        def post(self, group_id):
            """Create a post as a group"""
            current_user = g.current_user
            body = request.get_json()
            ...
            # Map Flask user to Spring user
            spring_user_id = get_spring_user_id(current_user)
            if not spring_user_id:
                return {'message': 'User not found in Spring backend'}, 404
            
            # Check group membership and permissions
            if not check_group_membership(spring_user_id, group_id):
                return {'message': 'You do not have permission to post to this group'}, 403
            ...
            # Handle topic creation/lookup (same as regular posts)
            if topic_path and not topic_id:
                try:
                    topic = Topic.get_or_create_for_page(
                        page_path=topic_path,
            ...
            try:
                # Create new micro blog post as group
                microblog = MicroBlog(
                    user_id=current_user.id,  # Keep original user for audit trail
                    content=content,
                    topic_id=topic_id,
                    data=data
                )
                
                # Add group metadata to data field
                if not microblog._data:
                    microblog._data = {}
                
                microblog._data['author_type'] = 'group'
                microblog._data['author_id'] = group_id
                microblog._data['posted_by_user_id'] = current_user.id  # Track who posted it
                
                created_microblog = microblog.create()
                if not created_microblog:
                    return {'message': 'Failed to create group post'}, 500
                
                # Enrich response with group info
                post_data = created_microblog.read()
                group_info = get_group_info(group_id)
                if group_info:
                    post_data['author'] = {
                        'type': 'group',
                ...
                return {'message': f'Error creating group post: {str(e)}'}, 500

    class _UserGroups(Resource):
        """Get groups where user can post"""
        
        @token_required()
        def get(self):
            """Get all groups where current user has posting permission"""
            current_user = g.current_user
            ...
            try:
                # Get user's groups from Spring
                response = requests.get(
                    f"{SPRING_URI}/api/groups/person/{spring_user_id}/groups",
                    timeout=5
                )
                ...
                if response.ok:
                    ...
                    # Filter to only groups where user can post
                    postable_groups = [
                        group for group in groups 
                        if group.get('canPost', True) or group.get('role') == 'admin'
                    ]
                    
                    return jsonify({
                        'groups': postable_groups,
                        'count': len(postable_groups)
                    })
                else:
                    return {'message': 'Failed to fetch groups from Spring'}, 500
                    
            except Exception as e:
                return {'message': f'Error retrieving user groups: {str(e)}'}, 500
...
# NEW: Group posting endpoints
api.add_resource(MicroBlogAPI._GroupPost, '/microblog/groups/<int:group_id>/posts', endpoint='microblog_group_post')
api.add_resource(MicroBlogAPI._UserGroups, '/microblog/user/groups', endpoint='microblog_user_groups')
